...
## 1. Краткое описание
Пользователь (продавец или аналитик) вводит ссылку на карточку товара на маркетплейсе (например, Wildberries, Ozon). Система автоматически парсит страницу товара, извлекает все отзывы, анализирует их с помощью ML-моделей, агрегирует результаты и предоставляет пользователю сводную аналитику по всем отзывам: общую тональность, рейтинг, динамику изменений, а также детальный разбор по ключевым аспектам товара.
...
```mermaid
graph TD
    A[Пользователь] -->|1. Вставляет ссылку| B(UI: Форма ввода URL)
    B -->|2. Запускает анализ| A
    A -->|3. Нажимает 'Анализировать'| B
    B -->|4. Валидирует URL| C{URL корректен?}
    C -->|Нет| D[Ошибка: Неверная ссылка]
    D -->|Показ ошибки| B
    C -->|Да| E[Backend: Менеджер задач]
    E -->|5. Запускает Парсер| F{Парсер маркетплейса}
    F -->|6. Извлекает отзывы| G{Отзывы найдены?}
    G -->|Нет| H[Ошибка: Нет отзывов]
    H -->|Показ сообщения| B
    G -->|Да| I[Backend: ML Анализатор]
    I -->|7. Анализ тональности и аспектов| J[Backend: Аггрегатор]
    J -->|8. Формирование дашборда| K[Успех: 200 OK]
    K -->|9. Показ дашборда| B
    B -->|10. Отображение результата| A

    subgraph "Обработка ошибок"
        L[Маркетплейс недоступен] -->|Ошибка парсера| M[Ошибка: 503]
        N[Слишком много запросов] -->|Ошибка: 429| O[Ошибка: Лимит]
        P[Сбой ML-модели] -->|Ошибка: 500| M
    end
