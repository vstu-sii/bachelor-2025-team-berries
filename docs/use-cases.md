...
# Use Case: Анализ отзывов по ссылке на товар - Sequence Diagram
...
sequenceDiagram
    ...
    Note over User,EmailService: Основной успешный сценарий
    
    User->>Frontend: 1. Нажимает "Проанализировать по ссылке"
    Frontend-->>User: 2. Открывает форму ввода URL
    ...
    Frontend->>Backend: 7. POST /api/analyze {url}
