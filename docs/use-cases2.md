...
```mermaid
graph TD
    A[Пользователь] -->|1. Переходит в 'Мои анализы'| B(UI: Список отчетов)
    B -->|2. Показывает список| A
    A -->|3. Кликает на отчет| B
    B -->|4. Запрос деталей отчета| C{Backend: База данных}
    C -->|5. Поиск отчета по ID| D{Отчет найден?}
    D -->|Нет| E[Ошибка: 404]
    E -->|Показ ошибки| B
    D -->|Да| F[Успех: 200 OK]
    F -->|6. Возврат данных отчета| B
    B -->|7. Открытие страницы отчета| G(UI: Детальный отчет)
    G -->|8. Показ отчета| A

    subgraph "Обработка ошибок"
        H[Нет анализов] -->|Пустой список| I[Показ экрана онбординга]
        J[Нет сети] -->|Ошибка сети| K[Показ оффлайн-уведомления]
    end
