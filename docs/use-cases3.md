...
- **Система email-рассылок:** Внешняя система для отправки подтверждающих писем и уведомлений.
...
| 7 | ... | Система показывает сообщение: "Письмо с подтверждением отправлено на ваш email. Проверьте почту." |
| 8 | Пользователь переходит по ссылке из email. | Система проверяет token, подтверждает email и автоматически авторизует пользователя. |
| 9 | ... | Система перенаправляет пользователя в личный кабинет с приветственным сообщением. |
